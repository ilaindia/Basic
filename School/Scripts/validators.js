function initializeAspNetValidationControls() {
    ...
    */
    ...
    SetValidatorCallouts = function () {
        ...
        for (i = 0; i < Page_Validators.length; i++) {
            ...
            if (!Page_Validators[i].isvalid) {
                //            if(pageValid)
                //                inputControl.focus();
                WebForm_AppendToClassName(inputControl, 'validation_error');
                pageValid = false;
            }
        }
        return pageValid;
    ...
    ClearValidatorCallouts = function () {
        ...
        for (i = 0; i < Page_Validators.length; i++) {
            var inputControl = document.getElementById(Page_Validators[i].controltovalidate);
            WebForm_RemoveClassName(inputControl, 'validation_error');
        }
    }

    ValidateRadTextBox = function (id) {
        var item = $("#" + id);
        if (item.attr('class').indexOf("rdfd_") != -1) {
            var pitem = item.parent("span");
            var txtBox = pitem.find("#" + id + "_text");
            //txtBox.css({
            //    "border-width": "1px",
            //    "border-style": "solid",
            //    "border-color": "red"
            //});
            console.log($(txtBox).prop('tagName'));
        }
    }
